...
class JS:
    ...
    def proxy_variable(self, **kwargs):
        for (k, v) in kwargs.items():
            ...
            self.__call__("""%s = mk_proxy("%s");""" % (k, k))
...
if __name__ == '__main__':
    ...
    # js.interp.add_callable("print",print)
    js.proxy_variable(print=print)
    # js.proxy_variable(request=requests.get)
    import requests

    js.interp.add_callable("request", requests.get)
    js('text=request("http://www.baidu.com").text;print(text)')
